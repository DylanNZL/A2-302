...
void initMembershipFunctions(fuzzy_system_rec *fl) {
  ...
  // Between -2.4m & 2.4m
  fl->inp_mem_fns[in_x][in_nl] = init_trapz( 0,     0,   -2,    -1.5,  left_trapezoid);    //
  fl->inp_mem_fns[in_x][in_ns] = init_trapz(-2,    -1.5, -0.75, -0.5, regular_trapezoid); //
  ...
  fl->inp_mem_fns[in_x_dot][in_nl] = init_trapz( 0,    0,   -3,    -2,  left_trapezoid);    //
  fl->inp_mem_fns[in_x_dot][in_ns] = init_trapz(-3,   -2,   -1,   -0.5, regular_trapezoid); //
  fl->inp_mem_fns[in_x_dot][in_ze] = init_trapz(-1,   -0.5,  0.5,  1,   regular_trapezoid); //
  fl->inp_mem_fns[in_x_dot][in_ps] = init_trapz( 0.5,  1,    2,    3,   regular_trapezoid); //
  ...
  fl->inp_mem_fns[in_theta][in_nl] = init_trapz( 0,    0,   -0.6, -0.5, left_trapezoid);    // -25 to -infinity
  fl->inp_mem_fns[in_theta][in_ns] = init_trapz(-0.5, -0.6, -0.2, -0.1, regular_trapezoid); // -25 to -5 degrees
  fl->inp_mem_fns[in_theta][in_ze] = init_trapz(-0.2, -0.1,  0.1,  0.2, regular_trapezoid); // -5 to 5 degrees
  fl->inp_mem_fns[in_theta][in_ps] = init_trapz( 0.1,  0.2,  0.5,  0.6, regular_trapezoid); //  5 to 25 degrees
  fl->inp_mem_fns[in_theta][in_pl] = init_trapz( 0.5,  0.6,  0,    0,   right_trapezoid);   // 25 to infinity
  /* The theta dot membership functions */
  // Between -0.4 & 0.4 Radians/Seconds (22 degrees)
  fl->inp_mem_fns[in_theta_dot][in_nl] = init_trapz( 0,     0,    -0.35, -0.3,  left_trapezoid);    //
  fl->inp_mem_fns[in_theta_dot][in_ns] = init_trapz(-0.35, -0.3,  -0.15, -0.1,  regular_trapezoid); //
  fl->inp_mem_fns[in_theta_dot][in_ze] = init_trapz(-0.15, -0.1,   0.1,   0.15, regular_trapezoid); //
  fl->inp_mem_fns[in_theta_dot][in_ps] = init_trapz( 0.1,   0.15,  0.3,   0.35, regular_trapezoid); //
  fl->inp_mem_fns[in_theta_dot][in_pl] = init_trapz( 0.3,   0.35,  0,     0,    right_trapezoid);   //
  return;
}
...
void initFuzzySystem (fuzzy_system_rec *fl) {
  ...
  fl->output_values[out_nm]  = -10.0;
	fl->output_values[out_ns]  = -0.5;
  fl->output_values[out_ze]  =  0.0;
	fl->output_values[out_ps]  =  0.5;
